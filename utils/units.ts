...
export const jsonStr = (str: string | any) => {
...
export function add(arg1: number, arg2: number) {
  ...
  if (gap > 0) {
    ...
    if (dotLength1 < dotLength2) int1 = int1 * gapMultiple;
    else int2 = int2 * gapMultiple;
  }
  ...
  if (regexArr === null) {
    ...
    if (regexArr[3] !== "-") {
      ...
      if (multiple >= dotLength) {
        /* 没有小数 */
        for (i = 0; i < gap; i++) tempArr.push("0");
      ...
      if (multiple === 0) {
        ...
        for (i = 0; i < multiple - 1; i++) tempArr.push("0");
...
export function judgeRectCoincidence(box1: Element, box2: Element) {
  ...
  const bx1 =
    box1?.getBoundingClientRect().x + box1?.getBoundingClientRect().width;
  const by1 =
    box1?.getBoundingClientRect().y + box1?.getBoundingClientRect().height;
  const b2x = box2?.getBoundingClientRect().x;
  const b2y = box2?.getBoundingClientRect().y;
  const b2x1 =
    box2?.getBoundingClientRect().x + box2?.getBoundingClientRect().width;
  const b2y1 =
    box2?.getBoundingClientRect().y + box2?.getBoundingClientRect().height;
...
export function clickOutside(ref: HTMLElement, cb = () => {}) {
  if (!ref) return;

  function handler(event: any) {
    if (ref && !ref.contains(event.target)) cb();
  }
...
export const initVConsole = () => {
  ...
  return new Promise((resolve) => {
    ...
    script.src =
      "https://cdn.bootcdn.net/ajax/libs/vConsole/3.11.1/vconsole.min.js";
...
export const formatNickname = (str: string | any[], max = 6) => {
  const arr = Array.from(str); // ["非", "拉", "Y", "非", "拉"]
  const len = arr?.length || 0;
  return len > max ? `${arr.slice(0, max).join("")}` : str;
};

const writeClipImg = async (blob: any) => {
  try {
...
export const copyDivToImage = (eid: string) => {
  const element = document.getElementById(eid || "__nuxt");
  if (!element) {
    console.log("暂无元素");
    return;
  }
...
export const domToImg = (eid: string) => {
  const element = document.getElementById(eid || "__nuxt");
  if (!element) {
    console.log("暂无元素");
    return;
  }
  domtoimage.toBlob(element).then(function (blob: any) {
    writeClipImg(blob);
    console.log("图像已成功复制到剪贴板");
